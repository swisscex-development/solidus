...
const string strMessageMagic = "Solidus Signed Message:\n";
...
{
    // Solidus: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Solidus: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Solidus: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Solidus: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Solidus: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ECUREX 30/Feb/2015 P2PFISY in Frankfurt";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Solidus: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SolidusMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SolidusMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SolidusMiner(CWallet *pwallet)
{
    printf("SolidusMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("solidus-miner");
    ...
    try { loop {
        ...
        printf("Running SolidusMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SolidusMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SolidusMiner, pwallet));
}
